...
class FoundationStereo(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False, low_memory=False, init_disp=None):
        ...
        coords = (
            torch.arange(w, dtype=torch.float, device=init_disp.device).reshape(1, 1, w, 1).repeat(b, h, 1, 1)
    ...
    def run_hierachical(self, image1, image2, iters=12, test_mode=False, low_memory=False, small_ratio=0.5):
        ...
        init_disp = (
            F.interpolate(disp_small_up, scale_factor=0.25, mode="bilinear", align_corners=True) * 0.25
        )  # Init disp will be 1/4
        disp = self.forward(
            image1, image2, iters=iters, test_mode=test_mode, low_memory=low_memory, init_disp=init_disp
        )
