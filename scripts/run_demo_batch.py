...
class CuSFMDataInference:
    ...
    def load_metadata(self):
        with open(self.args.metadata_file) as f:
    ...
    def process_camera(self, left_param_id: str):
        ...
        for synced_sample_id, left_keyframe in tqdm(
            left_camera_keyframes.items(), desc=f"Estimating depth for {camera}", leave=True
        ):
            ...
            if create_dir:
                os.makedirs(
                    os.path.dirname(
                        self.get_output_image_file(f"{self.args.out_dir}/scaled_0_4", left_keyframe["image_name"])
                    ),
                    exist_ok=True,
                )
                os.makedirs(
                    os.path.dirname(
                        self.get_output_image_file(f"{self.args.out_dir}/original_size", left_keyframe["image_name"])
                    ),
                    exist_ok=True,
                )
                if getattr(self.args, "also_generate_for_right_camera", False):
                    os.makedirs(
                        os.path.dirname(
                            self.get_output_image_file(f"{self.args.out_dir}/scaled_0_4", right_keyframe["image_name"])
                        ),
                        exist_ok=True,
                    )
                    os.makedirs(
                        os.path.dirname(
                            self.get_output_image_file(
                                f"{self.args.out_dir}/original_size", right_keyframe["image_name"]
                            )
                        ),
                        exist_ok=True,
                    )
            ...
            left_file = os.path.join(self.args.imgdir, left_keyframe["image_name"])
            right_file = os.path.join(self.args.imgdir, right_keyframe["image_name"])

            if not os.path.exists(left_file) or not os.path.exists(right_file):
                logging.info(f"Image not found for {left_file} or {right_file}")
                continue
            ...
            img0_tensor = torch.as_tensor(img0).to(self.device).float()[None].permute(0, 3, 1, 2)
            img1_tensor = torch.as_tensor(img1).to(self.device).float()[None].permute(0, 3, 1, 2)
            ...
            with torch.cuda.amp.autocast(True):
                disp = self.model.forward(img0_tensor, img1_tensor, iters=self.args.valid_iters, test_mode=True)
                ...
                if getattr(self.args, "also_generate_for_right_camera", False):
                    ...
                    disp2 = self.model.forward(
                        img1_flipped_tensor, img0_flipped_tensor, iters=self.args.valid_iters, test_mode=True
                    )
                    disp2 = torch.flip(disp2, dims=[3])
                    disp2 = padder.unpad(disp2.float())

            disp = disp.data.cpu().numpy().reshape(H, W)
            disp_big = cv2.resize(disp, (W_big, H_big), interpolation=cv2.INTER_NEAREST)
            ...
            imageio.imwrite(
                self.get_output_image_file(f"{self.args.out_dir}/original_size", left_keyframe["image_name"]),
                depth_big.astype(np.uint16),
            )

            if getattr(self.args, "also_generate_for_right_camera", False):
                disp2 = disp2.data.cpu().numpy().reshape(H, W)
                disp2_big = cv2.resize(disp2, (W_big, H_big), interpolation=cv2.INTER_NEAREST)

                depth2 = 1000 * scale * K[0, 0] * baseline / (disp2 + doffs)
                depth2_big = 1000 * scale * K[0, 0] * baseline / (disp2_big + doffs)
                ...
                imageio.imwrite(
                    self.get_output_image_file(f"{self.args.out_dir}/original_size", right_keyframe["image_name"]),
            ...
            if self.args.get_pc:
                ...
                pcd = self.toOpen3dCloud(xyz_map.reshape(-1, 3), img0.reshape(-1, 3))
                keep_mask = (np.asarray(pcd.points)[:, 2] > 0) & (np.asarray(pcd.points)[:, 2] <= self.args.z_far)
                keep_ids = np.arange(len(np.asarray(pcd.points)))[keep_mask]
                pcd = pcd.select_by_index(keep_ids)
                o3d.io.write_point_cloud(
                    f"{self.args.out_dir}/pcd/{camera}/{os.path.basename(left_file)[:-4]}.ply", pcd
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--imgdir", required=True, type=str, help="the directory to load camera images")
    parser.add_argument("--logdir", default="./debug", type=str, help="the directory to save logs and checkpoints")
    parser.add_argument("--ckpt_dir", default="../pretrained_models/23-51-11/model_best_bp2.pth", type=str)
    ...
    parser.add_argument("--get_pc", type=bool, default=False, help="get point cloud output")
    parser.add_argument("--z_far", default=10, type=float, help="max depth to clip in point cloud")
    parser.add_argument("--cameras", default=None, help="comma-separated camera names")
    parser.add_argument("--camera", default=None, type=str, help="single camera id (for subprocess usage)")
    parser.add_argument("--metadata_file", required=True, type=str, help="path to the metadata json file")
    parser.add_argument("--num_gpus", type=int, default=1, help="number of GPUs to use")
    parser.add_argument(
        "--also_generate_for_right_camera", type=bool, default=False, help="whether to generate depth for right camera"
    )
    ...
    if args.num_gpus > 1:
        mp.spawn(main, args=(args.num_gpus, args), nprocs=args.num_gpus, join=True)
    else:
        main(0, 1, args)
